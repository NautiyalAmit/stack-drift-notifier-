...
class SNSLogHandler(logging.Handler):
    def __init__(self, topic, subject, profile="tr-fr-sandbox"):
        logging.Handler.__init__(self)
        region = re.search('arn:\w+:\w+:(.+?):\d+:.+$', topic).group(1)
        self.session = boto3.session.Session(profile_name="tr-fr-sandbox",region_name="us-east-1")
...
class SNSlogger(object):
    def __init__(self, sns_topic_id, sns_topic_subject, profile="tr-fr-sandbox"):
